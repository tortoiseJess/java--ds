...
import pkg61bhw3.SList;
import pkg61bhw3.TestHelper;
...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int[] filteredArray = new int[ints.length];
    ...
    for (int i=0;i<ints.length;i++){
      ...
      filteredArray[0]=ints[0];
      int curIdx = 1;
      for (int i =0; i<=ints.length-2; i++){
          
          if (ints[i] != ints[i+1]) {
              filteredArray[curIdx] = ints[i+1];
              curIdx++;             
          }else{
              continue;
          }
  ...
  public static void main(String[] args) {
    ...
    pkg61bhw3.TestHelper.verify(result.equals(
