...
//          System.out.println(i+" th loop and current Node is "+currentNode.item);

          if (currentNode.item.equals(currentNode.next.item)) {
              currentNode.next= currentNode.next.next;
              // YOU FORGOT THE SIZE-- !!
//              currentNode = currentNode.next; WRONG!
          }else{
              currentNode = currentNode.next;
          }
          i++;
  ...
  public void twin() {
    ...
    // needa check for empty list
      /** my solution
      SListNode curNode = head;
      if (curNode == null){
          return;
      }
      SListNode nexNode = curNode.next;
      int i =0; 
      //loop condition is node is NOT  the last, so the last Node is excluded from the loop body?!
      while(curNode.next!= null){
          System.out.println(i+" th loop and current Node is "+curNode.item+" and nexNode is "+ nexNode.item);
      ...
      //deal with the last node, which is the curNode after the while loop, not the nexNode!
      curNode.next = new SListNode(curNode.item, null
      * **/
    SListNode currNode = head;
    while(currNode != null) {
      System.out.println("starting of loop current Node is "+currNode.item);
      SListNode dupNode = new SListNode(currNode.item, currNode.next);
      currNode.next = dupNode;
      currNode = dupNode.next;
      this.size++;
//      System.out.println("end of loop current Node is "+currNode.item);
